...
- **AI 教练（Packy GPT-5.6 Sol）**基于引擎证据解释失误、提出训练任务和复盘问题。
...
- 可选接入 Packy GPT-5.6 Sol，为局面损失超过 30 的走法生成结构化个性化讲解并保存到数据库；每次请求都包含截至当前的完整棋谱和此前悔棋分支
...
deepseek_coach.*         Packy GPT-5.6 Sol 异步请求、提示词、JSON 校验和本地引擎降级处理
...
推荐在项目根目录或程序目录放置一个名为 `APIKEY` 的本地文件，文件中只保存 Packy API 密钥；也可以设置 `PACKY_API_KEY` 或 `APIKEY` 环境变量。程序会使用 Packy 的 Responses 流式接口和 `gpt-5.6-sol`（高推理）完成单步讲解、追问和整盘复盘。`APIKEY` 已被 Git 忽略，不会进入提交或发布包；复制到另一台电脑时需要单独配置。

程序不会把 API 密钥写入源码、配置文件或数据库。也可以在程序的 **AI 设置** 中输入新的 Packy 密钥：
...
环境变量 `PACKY_API_KEY` 可用于开发和临时运行，并且优先于 Windows 凭据：

```powershell
$env:PACKY_API_KEY = "这里填写新密钥"
```
...
AI 教练统一使用 Packy 的 `gpt-5.6-sol`（高推理）生成单步讲解、追问和整盘复盘；没有 Packy 凭据时 AI 教练保持禁用，不会调用 DeepSeek。为了控制费用，优秀着法使用本地说明，只有局面损失超过 30 时才请求单步讲解。单步分析会携带从开局到当前的正式棋谱、既有引擎结论和全部悔棋分支，并强制输出具体线路、对手回应、推荐着真实目的和实战判定标准，禁止“重做三次”“落子前自问”等模板话。整盘复盘包括完整着法序列、胜负、阶段统计、关键转折点、思考时间和全部悔棋证据。解析失败或网络错误会自动重试，仍失败时保存基于 Pikafish 数据的本地复盘。模型只负责解释皮卡鱼证据，不能修改引擎评分或编造变化。
