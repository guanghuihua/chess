...
AI 教练统一使用 DeepSeek 的 `deepseek-chat` 生成单步讲解、追问和整盘复盘；没有 DeepSeek 凭据时 AI 教练保持禁用。为了控制费用，优秀着法使用本地说明，只有局面损失超过 30 时才请求单步讲解。单步讲解和追问只会携带已验证的中文棋谱、实战惩罚线、推荐应对线、评分差异及可选的用户思路，不会发送原始棋盘编码、UCI 坐标或完整坐标历史；具体子力关系应在引擎变化回放棋盘中核对。整盘复盘会携带完整棋谱、胜负、阶段统计、关键转折点、思考时间和悔棋证据。AI 原创题仅接收压缩后的用户画像、当前训练计划、最多三条诊断证据和最近题目主题，生成后必须通过 XiangqiGame 与 Pikafish 验证才可入库。模型只负责解释和出题草案，不能修改引擎评分或编造未验证变化。
